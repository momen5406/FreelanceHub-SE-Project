...
$disputes = $controller->getMediatorDashboardData();
...
<div class="container py-4">
    <div class="mb-4">
        <h2 class="fw-bold mb-1" style="color:#1a1a2e;">Dispute Dashboard</h2>
        <p class="text-muted mb-0">Review and resolve disputes across the platform.</p>
    </div>
            ...
            <table class="table mb-0">
                <thead style="background:#f5f4f0;">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-muted">Raised By</th>
                        <th class="px-4 py-3 text-muted">Against</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td colspan="6" class="px-4 py-5 text-center text-muted">No disputes found.</td>
                    </tr>
                    <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-3"><?= htmlspecialchars($dispute['raised_by_name'] ?? 'Unknown') ?></td>
                        <td class="px-4 py-3"><?= htmlspecialchars($dispute['against_user_name'] ?? 'Unknown') ?></td>
                        <td class="px-4 py-3"><span
                                class="status-badge <?= $statusClass ?>"><?= htmlspecialchars($dispute['status']) ?></span>
                        </td>
                        <td class="px-4 py-3 text-muted"><?= date('M d, Y', strtotime($dispute['created_at'])) ?></td>
                        <td class="px-4 py-3 text-end"><a href="view.php?id=<?= (int)$dispute['id'] ?>"
                                class="btn btn-sm btn-fh-primary">View</a></td>
                    </tr>
                    <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
